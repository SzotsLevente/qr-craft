package com.example.qrcraft.ui.theme

import androidx.compose.ui.graphics.Color

// Primary Colors
val Primary = Color(0xFFEBFF69)
val Surface = Color(0xFFEDF2F5)
val SurfaceHigher = Color(0xFFFFFFFF)
val OnSurface = Color(0xFF273037)
val OnSurfaceAlt = Color(0xFF505F6A)
val Overlay = Color(0x80000000) // 50% opacity
val OnOverlay = Color(0xFFFFFFFF)

// QR Code Type Colors
val Link = Color(0xFF373F05)
val LinkBG = Color(0x4DEBFF69) // EBFF69 with 30% opacity
val Error = Color(0xFFF12244)
val Success = Color(0xFF4DDA9D)
val Text = Color(0xFF583DC5)
val TextBG = Color(0x1A583DC5) // 583DC5 with 10% opacity
val Contact = Color(0xFF259570)
val ContactBG = Color(0x1A259570) // 259570 with 10% opacity
val Geo = Color(0xFFB51D5C)
val GeoBG = Color(0x1AB51D5C) // B51D5C with 10% opacity
val Phone = Color(0xFFC86017)
val PhoneBG = Color(0x1AC86017) // C86017 with 10% opacity
val WiFi = Color(0xFF1F44CD)
val WiFiBG = Color(0x1A1F44CD) // 1F44CD with 10% opacity